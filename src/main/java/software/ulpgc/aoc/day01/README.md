...
*   **Abstracción**:
    ...
    *   *Implementación*: El record `DialStatus` representa únicamente el estado físico del dial, y oculta la complejidad matemática de desbordamientos a través de métodos públicos simples.
*   **Encapsulamiento**:
    *   *Definición*: El código esconde su complejidad interna, mostrándose al exterior mediante una interfaz más simple de operar.
    *   *Implementación*: `DialStatus` protege la lógica interna de actualización del estado, exponiendo únicamente la posición del dial. Por su parte, `DialCalculator` encapsula la lógica matemática y de optimización para calcular los cruces por cero.
*   **Cohesión**:
    ...
    *   *Implementación*: Cada componente tiene una única responsabilidad bien enfocada. El record `Order` solo contiene el dato del paso, `Dial` representa exclusivamente la identidad del marcador, `DialStatus` procesa de forma exclusiva el posicionamiento físico del dial, y `DialCalculator` se ocupa de las métricas de conteo solicitadas por el problema.
*   **Bajo acoplamiento**:
    *   *Definición*: Las dependencias entre módulos son mínimas y se basan en abstracciones.
    *   *Implementación*: El flujo principal (`Main`) utiliza la factoría genérica `LoaderFactory` del paquete `common.io` para leer el fichero, lo que le permite cambiar el formato o el cargador de datos sin afectar en absoluto a las clases de modelo.
*   **Código expresivo**:
    *   *Definición*: El código es claro y fácil de entender.
    *   *Implementación*: El uso de **Records** inmutables y la programación funcional con **Java Streams** permiten que los algoritmos se lean de forma declarativa, haciendo innecesarios los comentarios aclaratorios.
...
*   **Law of Demeter (LoD - Ley de Deméter)**:
    ...
    *   *Implementación*: La clase `Main` interactúa únicamente con `Dial` y `DialStatus` directamente sin navegar por las propiedades de caracteres individuales del record `Order`.
*   **Principio de mínimo compromiso**:
    ...
    *   *Implementación*: Los métodos públicos de `DialStatus` (`position()`, `countEndingInZero()`, `countCrossingZero()`) ocultan los algoritmos intermedios y de optimización de desbordamientos de cruces por cero.
...
*   **Inmutabilidad del modelo**:
    *   *Definición*: Las clases del modelo se definen como Records, asegurando que sus instancias sean totalmente inmutables una vez creadas, lo que favorece la abstracción.
...
*   **Programación funcional (con Java Streams)**:
    *   *Definición*: Paradigma de programación basado en la composición y aplicación de funciones, donde las operaciones se expresan de forma declarativa, describiendo qué se quiere obtener en lugar de cómo realizar cada paso.
    *   *Implementación*:
...
*   **Good Naming**:
    ...
    *   *Implementación*: Nombres explícitos como `INITIAL_POSITION`, `calculateZerosCrossed()`, o `countEndingInZero()` aseguran que el código sea autodocumentado.
...
*   **Patrones creacionales**:
    *   **Factory Method**:
        ...
        *   *Implementación*: 
            *   [Dial.java:L11-L16](https://github.com/lauraheerrera/aoc2025/blob/master/src/main/java/software/ulpgc/aoc/day01/model/Dial.java#L11-L16) mediante el método estático `Dial.create()`. Este método encapsula la construcción del dial y ofrece un punto de entrada limpio en el punto de uso.
            *   [DialStatus.java:L18-L20](https://github.com/lauraheerrera/aoc2025/blob/master/src/main/java/software/ulpgc/aoc/day01/model/DialStatus.java#L18-L20) mediante el método estático `DialStatus.initial(Dial)`. Este método encapsula la construcción del estado inicial del dial y ofrece un punto de entrada limpio en el punto de uso.
